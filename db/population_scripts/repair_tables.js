...
function genRepairInvoice() {
    let repairInvoice = [];
    let labourCost = 45.00
    repairInvoice.push(randInt(50)+1);     // member_id
    repairInvoice.push(randInt(10)+1);     // employee_id
    repairInvoice.push(randInt(5)+1);     // repair_status_id
    repairInvoice.push(randNth(repairDescription));     // repair_invoice_description
    repairInvoice.push(randInt(8)+1);      // repair_invoice_labour_hours
    repairInvoice.push(labourCost);        // repair_invoice_cost hard coded labour cost
...
function genRepairItem() {
    let repairItem = [];
    repairItem.push(randInt(10)+1);                     // repair_invoice_id
...
exports.seedRepairTables = function() {
    ...
    let insertRepairInvoiceSql = 'INSERT INTO tbl_repair_invoices(member_id, employee_id, ' +
    ...
    let repairStatus = Array.from({length: 5}, genRepairStatus);
